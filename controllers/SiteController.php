...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $posts = new ActiveDataProvider([
            ...
            'query'=>Post::find()->where('posted_by != 0 and posted_by != ""'),
            'pagination'=>[
                'pageSize'=>5,
            ]
        ]);
   ...
   /* public function isAdmin($post_id){
        $model=NewUser::find()->where(['id' => Yii::$app->user->id])->one();
        if ($model!=null){
            if(Post::findOne($post_id)->postedBy->id==0 || Post::findOne($post_id)->postedBy->id==$post_id)
            return true;
        } else {
            return false;
        }
